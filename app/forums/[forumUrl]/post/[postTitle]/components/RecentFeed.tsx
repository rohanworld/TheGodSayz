...
<Table>
      <TableHeader>
        <TableRow>
          <TableHead className=" text-left font-style-0 font-jakarta  dark:text-white">Latest Posts</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {posts.map((post, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="font-style-1-descriptions font-jakarta  text-[14px]">{post.description.length>1000?parse(post.description.substring(0, 99))+"...":parse(post.description)}</TableCell>
            </Link>
            {index!=4&&
            <Separator/>
}
