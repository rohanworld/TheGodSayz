...
export function AlbumArtwork({ post, isProfile = false, handleDelete = () => {} }: Props) {
  ...
  return (
    <div className="dark:bg-[#262626] mb-4 font-style-6-post font-jakarta h-[22.4rem] w-[21rem] bg-white rounded-lg transition-all duration-300">
    <div className="lg:w-[18.5rem] w-[full] lg:h-[8.1rem] h-[7.7rem]">
      <ContextMenu>
        <ContextMenuTrigger>
          ...
          >
            <Bookmark
              className={cn("h-4 w-4 text-white font-style-4 font-dmsans ", {
          ...
          <div className="pt-[9px] absolute top-[0rem] left-[0.5rem]">
          {
            post?.category?.slice(0, 2).map((category, index)=>(
              <Button key={index} className='font-style-4 font-dmsans  text-black bg-slate-100 hover:text-white rounded-3xl h-[1.4rem] mr-[0.4rem] px-2'>
                            {category.split("|").join("/")}
                            </Button>
            ))
}                 
      ...
      <Link href={`/event-details/${encodeURIComponent(post?.title?.split(" ").join("-"))}`} prefetch >
        <h3 className="font-style-2 font-dmsans  text-[18px] leading-none ml-5 mt-5">{post?.title?.length>28?post.title.substring(0, 27)+"...":post.title}</h3>
      </Link>  
      {/* <div className="mt-[0.30rem] text-[14px] font-semibold opacity-85">{post.locationOfEvent}</div> */}
      <div className="lg:block">
        {
        <div className="m-5 font-style-4 font-dmsans  flex gap-[25px]"><span className="text-black w-2 mt-[1.5px]"><CalendarDays className="text-xs w-4 h-4"/></span>{dateString}</div>
}
        </div>
        <div className="flex bg-[#e8e8e8] w-[21rem] px-5 py-3 mt-[33px] rounded-b-lg">
          <div>
            <div className="font-style-4 font-dmsans  w-[8rem]">Organized By</div>
            <div className="font-style-4 font-dmsans ">{post.name}</div>
          </div>
          <div className="ml-[73px] font-style-4 font-dmsans  font-[700]"><Button className="bg-white rounded-3xl hover:text-white text-[#000000] border-[#000000] border-2 font-[700]"><div className="font-[700]">BUY NOW</div></Button></div>
        </div>
      </div>
    </div>
    </div>
