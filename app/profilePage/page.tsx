...
useEffect(() => {
  const fetchSavedEvents = async () => {
    ...
    if (userDoc.exists()) {
      ...
      const finalEventList = savedEvents.filter(event => event.title && event.title.trim() !== "");
      //console.log("SE: ", savedEvents);
      setSavedEvents(finalEventList);
    }
  };
  ...
  <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{isAnonymous ? 'Anonymous Posts' : 'Normal Posts'}</span>
      ...
      <TabsList className="grid gap-2 grid-cols-8 w-[800px]">
