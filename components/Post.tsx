...
const Post = ({ post, isProfile = false, othersProfile , handleDelete = () => {} }: Props) => {
  ...
  return (
    <>
      {post.uid ? (
        <div className="bg-[#ffffff] dark:bg-[rgb(75,63,63)] font-style-6-post font-jakarta mb-3 my-[14px] fade-in">
          <div className="px-6 py-4 flex justify-between space-x-4 rounded-[4px]">
            <div className="w-0 flex-1 break-normal overflow-hidden">
              <Link href={`/${isPoll ? `poll/${encodeURIComponent(post?.title?.split(" ").join("-"))}` : encodeURIComponent(post?.title?.split(" ").join("-"))}`} prefetch>
                {!isProfile && (
                  <div className="flex max-h-40 mt-1 space-x-2 text-xs">
                    ...
                    <div className="flex space-x-2">
                      <span className="font-style-5-username font-dmsans  mt-3 text-[#0c0c0c] dark:text-yellow-50">
                        {isAnonymous ? (
                          "Anonymous"
                        ) : (
                      ...
                      {isAnonymous || isCurrentUser || !user ? null : (
                        <div className="flex space-x-1 mr-5">
                          ...
                          <button className="font-style-4 font-dmsans  text-[14px] mt-[0.33rem] text-blue-500 p-0 hover:underline cursor-pointer" onClick={handleFollow}>
                ...
                <div className={`relative max-h-20 w-full overflow-clip ${isExpanded ? "max-h-none expand-content" : ""}`} ref={pRef}>
                  <p onClick={fetchQuestion} className="font-style-3 font-jakarta  dark:text-white">
                    ...
                    return (
                      <div key={index} className="w-[95%] flex gap-2 items-center">
                        <div className="relative w-full">
                          ...
                          <div className="absolute top-[11px] left-4 font-style-3-vote font-jakarta">{optionKey}<span className="ml-[1rem] text-blue-500">{`${progressValue.toFixed(1)} % Votes`}</span></div>
                        </div>
                        <div>
                          {user?.isAnonymous ? (
                            <Button className="rounded-3xl font-style-3-vote font-jakarta border-2 text-center border-black h-[2rem] bg-transparent hover:text-black text-black hover:bg-[#e4dcdc]" onClick={() => {
                              toast({
                            ...
                            <Button className={`rounded-3xl border-2 font-style-3-vote font-jakarta text-center text-white hover:text-white border-black h-[2rem] ${userVote == optionKey ? "bg-black" : "bg-transparent text-black hover:text-black hover:bg-[#e4dcdc]"}`} onClick={() => handleVote(optionKey)}>Vote</Button>
                          )}
                        </div>
                      </div>
                    );
            ...
            <div className="flex-col space-y-6 items-center justify-center my-auto">
              <button className="font-style-4 font-dmsans  w-fit flex items-center gap-2">
              ...
              <button className="w-fit flex items-center gap-2" onClick={handleSave}>
                <Bookmark className={`font-style-4 font-dmsans  h-4 w-4 font-style-4 font-dmsans  ${savedState ? "text-black fill-black" : ""}`} />
                {savedState ? (
                  <span className="font-style-4 font-dmsans  sm:block hidden text-black dark:text-white">Saved</span>
                ) : (
                  <span className="font-style-4 font-dmsans  sm:block hidden text-black dark:text-white">Save</span>
                )}
              </button>
            </div>
          </div>
          <div className="rounded-b-md bg-[#e7e7e7] dark:bg-[#1A1A1B]/65 z-20 flex gap-x-3 text-sm px-3 py-3 sm:px-6">
            ...
            <div className="ml-2 mt-2 flex justify-between w-full">
              <Link href={`/${isPoll ? `poll/${encodeURIComponent(post?.title?.split(" ").join("-"))}` : encodeURIComponent(post?.title?.split(" ").join("-"))}`} className="w-fit flex items-center gap-2">
                ...
                <span className="font-style-4 font-dmsans  sm:block hidden postvotec text-black dark:text-white">
