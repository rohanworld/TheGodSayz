...
const Navbar = ({}: Props) => {
  ...
  const [navSearchText, setNavSearchText] = useState<any>("");
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
        if (!user?.uid) {
            return; // Exit early if user or user.uid is undefined
        }
...
const clearNotifications = async () => {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    if (
      ...
      !suggestionsContainerRef.current.contains(e.relatedTarget as Node)
    ) {
      setSuggestions([]);
      setNavSearchText("");
    }
...
{hideQueSearch&&<>
    <Link href={'/events/createEvent'}>
      ...
      {searchResult?.length > 0 && (
        <div ref={suggestionsContainerRef} className="absolute mt-2 w-full bg-white opacity-95 rounded-md shadow-lg">
          {searchResult.map((hit:any) => (
            <Link key={hit.objectID} onMouseDown={(e) => e.preventDefault()} className="w-full h-full" href={`/${hit.options ? `poll/${encodeURIComponent(hit?.title?.split(" ").join("-"))}` : encodeURIComponent(hit?.title?.split(" ").join("-"))}`} prefetch>
            <div  className="p-2 hover:bg-gray-200 opacity-95 cursor-pointer hover:rounded-md">
