...
import PollsCommentPost from "./PollsCommentPost";
  ...
  const loadMoreAnswers = async () => {
    ...
    if (!ansSnapshot.empty) {
      const newAnswers = await Promise.all(
        ansSnapshot.docs.map(async (doc) => {
          const answerData = doc.data() as AnswerType;
          const commentsQuery = collection(db, "pollsAnswers", doc.id, "comments");
          const commentsSnapshot = await getDocs(commentsQuery);
        ...
        dispAnswer.map((answer: any, key) => (
          ...
          >
            <div className="px-6 py-5 flex justify-between">
              <div className="w-0 flex-1">
                ...
                <div className="flex gap-3">
                  ...
                  <span className="mt-3 font-style-5-username font-dmsans  font-semibold dark:text-white">
                    {answer.anonymity ? (
                      "Anonymous"
                    ) : (
                      <Link
                  ...
                  answer.uid === user.uid ? null : (
                    <div className="flex space-x-1 mr-5">
                      ...
                      <div>
                        ...
                        >
                          <span className="sm:block hidden font-style-5-username font-dmsans  text-blue-500">
                            {isUserFollowed(answer.uid)
                              ? "Following"
                              : "Follow"}
                          </span>
                  ...
                  <div className="mt-[11px]">{answer?.createdAt?.toDate()?.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                </div>
                ...
                <div
                  ...
                  ref={pRef}
                >
                  <p className="ProseMirror font-style-3 font-jakarta ">
            ...
            <div className="dark:bg-[#1A1A1B]/65 z-20 flex justify-between gap-x-3 text-sm px-4 py-4 sm:px-6">
              <PostVoteClientPhone
                postId={answer.id}
                postType="pollsAnswers"
                userId={user?.uid!}
                questionId={postId}
              />
              <div className="flex gap-x-3">
                <button
                  className="font-style-4 font-dmsans  w-fit flex items-center gap-2"
                  onClick={() => toggleCommentInputVisibility(key)}
                >
                  <MessageSquare className="font-style-4 font-dmsans  h-4 w-4" />{" "}
                  <span className="sm:block hidden">
                    {answer.comments} Comments
                  </span>
                </button>
                <button className="w-fit font-style-4 font-dmsans  flex items-center gap-2">
                <ShareDialog postLink={`/poll/${postTitleWithSpaces.split(" ").join("-")}`}/>
                </button>
                <button className="w-fit flex items-center gap-2">
                  <Bookmark className="font-style-4 font-dmsans  h-4 w-4" />{" "}
