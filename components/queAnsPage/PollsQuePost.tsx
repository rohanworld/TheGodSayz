      ...
      <div className="px-6 md:py-6 py-4 flex justify-between">
        ...
        <div className="w-0 flex-1">
          <div className="flex justify-between max-h-60 mt-1 space-x-2 text-xs">
            ...
            {isAnonymous||isCurrentUser || !user ? null : (
              <div className=" flex space-x-1 mr-5">
                ...
                <div
                  className=" text-blue-500 font-style-5-username font-dmsans  mt-[0.87rem] p-0 hover:underline cursor-pointer"
          ...
          {/* <a href={`/postPage/${post.id}`}> */}
            <h1 className="font-style-4 font-dmsans  py-3 leading-6 dark:text-white">
              {post.title}
            </h1>
          {/* </a> */}
          ...
          <div
            className="relative max-h-50 w-full  "
            ref={pRef}
          >
            {/* <EditorOutput content={post.content} /> */}
            <p className="ProseMirror font-style-3 font-jakarta  dark:text-white">{post.description ? parse(post.description) : ""}</p>
          ...
          <div>
            ...
            <div className="absolute top-[11px] font-style-3-vote font-jakarta left-4">{optionKey}<span className="ml-[1rem] text-blue-500">{`${progressValue.toFixed(1)} % Votes`}</span></div>
            </div>
            <div>
              {
                user?.isAnonymous?<Button className={`rounded-3xl border-2 text-center font-style-3-vote font-jakarta border-black h-[2rem] bg-transparent text-black hover:text-black hover:bg-[#b6b8bd]`} onClick={() => {toast({
                  title: "Sign in to Vote",
                  variant: "destructive",
                });}}>Vote</Button>:<Button className={`font-style-3-vote font-jakarta rounded-3xl text-white hover:text-white border-2 text-center border-black h-[2rem] ${userVote==optionKey?"bg-black":"bg-transparent text-black hover:text-black hover:bg-[#e4dcdc]"}`} onClick={() => handleVote(optionKey)}>Vote</Button>
              }
            </div>
            </div>
              })
