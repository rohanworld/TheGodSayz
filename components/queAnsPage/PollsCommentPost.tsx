...
const PollsCommentPost = ({
  ...
  async function createCommentPost(data: Input) {
    ...
    if (!snapshot.empty) {
      ...
      const docRef = await addDoc(
        collection(db, "pollsAnswers", answerId, "comments"),
        {
  ...
  async function submitReply(index: number) {
    ...
    if (!snapshot.empty) {
      ...
      const commentRef = docc(db, `pollsAnswers/${answerId}/comments/${commentId}`);
        ...
        {comments.map((comment, index) => (
          ...
          >
            <div className="flex  space-x-2">
              ...
              />
              <div>
                <p className="text-sm font-semibold font-style-5-username font-dmsans ">{comment.name}</p>
                <p className="text-sm">{comment.comment}</p>
                <div>
                  <button
                    onClick={() => toggleReplyInputVisibility(index)}
                    className="font-style-4 font-dmsans  text-blue-500 hover:underline"
                  >
                    Reply
                  </button>
...
export default PollsCommentPost;
