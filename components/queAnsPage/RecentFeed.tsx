...
<Table>
      <TableHeader>
        <TableRow>
          <TableHead className=" text-left font-style-0 font-jakarta dark:text-white ">Recent Posts</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {posts.map((post, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="font-style-2 font-dmsans  text-[14px] font-[500]">{post.title.length>70?post.title.substring(0, 69)+"...":post.title}</TableCell>
            <TableCell className="font-style-3 font-jakarta  text-[14px] text-zinc-500 dark:text-gray-400">{post.description.length>1000?parse(post.description.substring(0, 99))+"...":parse(post.description)}</TableCell>
            </Link>
